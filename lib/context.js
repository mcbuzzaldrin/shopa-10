 ...
 * (restores your original GROQ with cartPhotos)
 */
...
async function setCheckoutState(checkout, setContext, openCart) {
  ...
  const lineItems = await Promise.all(
    checkout.lineItems.map(async (item) => {
      const variantID = item.variant.id.split('gid://shopify/ProductVariant/')[1]
...
export function useTogglePageTransition() {
  const { context: { isPageTransition }, setContext } = useContext(SiteContext)
  return (state) => setContext(cs => ({ ...cs, isPageTransition: state }))
}

export function useToggleMegaNav() {
  const { context: { meganav }, setContext } = useContext(SiteContext)
...
export function useCartTotals() {
  const { context: { checkout } } = useContext(SiteContext)
  return { subTotal: checkout.subTotal * 100 }
}
...
export function useAddItem() {
  const { context: { checkout, shopifyClient }, setContext } =
...
export function useUpdateItem() {
  ...
  return async (lineID, qty) => {
    ...
    const newC = await shopifyClient.checkout.updateLineItems(
      checkout.id,
...
export function useRemoveItem() {
  ...
  return async (lineID) => {
    setContext(cs => ({ ...cs, isUpdating: true }))
    const newC = await shopifyClient.checkout.removeLineItems(
      checkout.id,
      [lineID]
    )
    setCheckoutState(newC, setContext, false)
